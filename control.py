    ...
    def periodic_updater(self):
        while True:
            ...
            self.send_information(
                '{"infraNodeId": "' + str(self.nodeId) + '", "alert" : "Weather alert", "senorId" : "WTR", "senorReading" : "' + random.choices(pedictions)[0] + '"}')
            time.sleep(10)
...
class VehicleControls(bs.BroadcastSystem):
    ...
    def process_gps_data(self, data):
        self.GPS = data[1]
        logging.info(f'[{self.vechile_id}] Broadcasting GPS signal')
    ...
    def process_tyre_pressure_data(self, data):
        ...
        if data[1] < 30 or data[1] > 35:
            ...
            self.send_information(
                '{"vehicleId": "' + str(self.vechile_id) + '", "alert" : "Low or high tyre pressure alert", "senorId" : "TP", "senorReading" : "' + str(data[1]) + '"}')
